...
class Layout():
    def __init__(self, G: nx.Graph, alg: str):
        assert alg in ["random", "stress", "tsnet", "sfdp", "neato", "twopi"]
    ...
    def tsnet_layout(self):
        ...
        # return TSNE(init="random", learning_rate='auto', metric="precomputed", perplexity=min(30, self.G.number_of_nodes()-1)).fit_transform(get_apsp(self.G))
    
    def sfdp_layout(self):
        pos = nx.nx_pydot.pydot_layout(self.G, prog='sfdp')
        return self.dict_to_mat(pos)
    
    def neato_layout(self):
        pos = nx.nx_pydot.pydot_layout(self.G, prog='neato')
        return self.dict_to_mat(pos)
    
    def twopi_layout(self):
        pos = nx.nx_pydot.pydot_layout(self.G, prog='twopi')
        return self.dict_to_mat(pos)
    ...
    def compute(self):
        if self.alg == "random":
            ...
            self.X = self.neato_layout()
        elif self.alg == "twopi":
            self.X = self.twopi_layout()
...
if __name__ == "__main__":
    import os
    import tqdm
    from modules.graph_io import load_txt, load_embedding

    if not os.path.isdir("embeddings"):
        os.makedirs("embeddings")

    for gname in tqdm.tqdm(os.listdir("Rome_graphs")):
        G = load_txt("Rome_graphs/" + gname)
